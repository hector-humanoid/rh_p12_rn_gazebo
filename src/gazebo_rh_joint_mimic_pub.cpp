...
int main(int argc, char **argv)
{
  ...
  std::string prefix = pnh.param("prefix", std::string());

  g_rh_r2_joint_pub = nh.advertise<std_msgs::Float64>(prefix + "rh_r2_position/command", 0);
  g_rh_l1_joint_pub = nh.advertise<std_msgs::Float64>(prefix + "rh_l1_position/command", 0);
  g_rh_l2_joint_pub = nh.advertise<std_msgs::Float64>(prefix + "rh_l2_position/command", 0);

  ros::Subscriber rh_joint_sub = nh.subscribe(prefix + "rh_p12_rn_position/command", 5, rhJointCallback);
